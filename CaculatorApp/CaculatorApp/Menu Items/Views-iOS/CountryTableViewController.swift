...
class CountryManager {
    ...
    private let countryList  = ["AF":"93","AL":"355","DZ":"213","US":"1",
    ...
    {
      if let key = countryList.first(where: { $0.value == country })?.key {
          return key
      }
      return ""
    }
    ...
    struct Country {
        var name: String {
            countryName(countryCode: nameCode) ?? "No Name found"
        }
        let nameCode: String
    ...
    public func countries() -> [Country] {
        allCountries = countryList.map{ Country(nameCode: $0, countryCode: $1, flag: flag(country: $0)) }
        return allCountries
    }

    public func getCountry(at index: Int) -> Country {
...
class CountryTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // Configure the cell...
        let country = CountryManager.shared.getCountry(at: indexPath.row)

        let number = Int.random(in: 0..<3)
        print("\(number) isZero: \(number.isZero)")
